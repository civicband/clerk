...
down_revision: str | Sequence[str] | None = "f3d4e5b6c7a8"
...
def upgrade() -> None:
    ...
    op.execute("UPDATE sites SET has_finance_data = 0 WHERE has_finance_data = 'false'")

    # Convert string 'true' to integer 1 (if any exist)
    op.execute("UPDATE sites SET has_finance_data = 1 WHERE has_finance_data = 'true'")

    # Also fix coordinator_enqueued if it has the same issue
    op.execute("UPDATE sites SET coordinator_enqueued = 0 WHERE coordinator_enqueued = 'false'")
    op.execute("UPDATE sites SET coordinator_enqueued = 1 WHERE coordinator_enqueued = 'true'")
