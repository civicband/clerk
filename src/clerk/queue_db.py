...
def track_job(conn, rq_job_id, site_id, job_type, stage):
...
def create_site_progress(conn, site_id, stage):
    ...
    if conn.dialect.name == "postgresql":
        ...
        stmt = stmt.on_conflict_do_update(index_elements=["site_id"], set_=data)
    else:
...
def update_site_progress(conn, site_id, stage=None, stage_total=None):
...
def increment_stage_progress(conn, site_id):
    ...
    stmt = (
        ...
        .where(site_progress_table.c.site_id == site_id)
        .values(
